...
struct Child : NoCopyNoMove
{
    ...
    {
        IsContentVisible = ImGui::BeginChild(str_id, size, child_flags, window_flags);
    }
    Child(ImGuiID id, const ImVec2 &size = ImVec2(0, 0), ImGuiChildFlags child_flags = 0, ImGuiWindowFlags window_flags = 0)
    {
        IsContentVisible = ImGui::BeginChild(id, size, child_flags, window_flags);
    }
...
struct PopupContextWindow : NoCopyNoMove
{
    ...
    PopupContextWindow(const char *str_id = NULL, ImGuiPopupFlags popup_flags = 1)
    {
